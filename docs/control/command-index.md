...
| `open_window` | Window操作 | `id`, `cmd`, `window_role`, `width`, `height`, optional `monitor_index`, `fullscreen` | `window_role`, `window_id`, `width`, `height` | `window_opened` | なし | なし | Window backend、Window placement backend | 実装済み（X11/niri。Win32未実装） | [projector.md](./commands/projector.md#open_window) |
