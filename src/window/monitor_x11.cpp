...
std::vector<MonitorRect> enumerate_monitors_x11()
{
    ...
    for (int i = 0; i < screen_resources->ncrtc; ++i)
    {
        ...
        {
            monitors.push_back(MonitorRect{static_cast<int>(crtc_info->x), static_cast<int>(crtc_info->y),
                                           static_cast<int>(crtc_info->width), static_cast<int>(crtc_info->height),
                                           [&]
                                           {
                                               if (crtc_info->noutput == 0)
                                                   return std::string{};
                                               XRROutputInfo* output =
                                                   XRRGetOutputInfo(display, screen_resources, crtc_info->outputs[0]);
                                               if (!output)
                                                   return std::string{};
                                               std::string name(output->name,
                                                                static_cast<std::size_t>(output->nameLen));
                                               XRRFreeOutputInfo(output);
                                               return name;
                                           }()});

            SPDLOG_INFO("モニタ {}: 位置=({}, {}), サイズ={}x{}", i, crtc_info->x, crtc_info->y, crtc_info->width,
                        crtc_info->height);
        }
        if (crtc_info)
