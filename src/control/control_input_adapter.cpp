...
AdapterResult ControlInputAdapter::handleCaptureFrameCommand(
    const std::string& id,
    const ControlMessage& message,
    bool calibration) {
  const auto map_result = calibration
      ? headless_mapper_.mapCalibrationCaptureFrame(message)
      : headless_mapper_.mapCaptureFrame(message);
  if (!map_result.ok) {
    writeHeadlessFailure(
        id,
        map_result.error.value_or(common::CommandError{
  ...
  if (result.handled && result.ok) {
    writer_.writeEvent(ControlEvent{
        calibration ? "calibration_frame_saved" : "frame_saved",
        {{"role", *message.role}, {"path", path}}});
  }
  return AdapterResult::continue_running;
}

AdapterResult ControlInputAdapter::handleCaptureStereoCommand(
    const std::string& id,
    const ControlMessage& message,
    bool calibration) {
  const auto map_result = calibration
      ? headless_mapper_.mapCalibrationCaptureStereo(message)
  ...
  if (!map_result.ok) {
    writeHeadlessFailure(
        ...
        map_result.error.value_or(common::CommandError{
            "invalid_command",
            "failed to map capture_stereo command"}));
    return AdapterResult::continue_running;
  }
  ...
  if (result.handled && result.ok) {
    writer_.writeEvent(ControlEvent{
        calibration ? "calibration_stereo_frame_saved" : "stereo_frame_saved",
