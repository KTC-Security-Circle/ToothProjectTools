...
bool readString(const cv::FileNode& root, const char* name, std::optional<std::string>& value,
...
bool readInteger(const cv::FileNode& root, const char* name, std::optional<int>& value, std::string& error_message)
{
    const auto node = root[name];
    if (node.empty())
    {
...
bool readNumber(const cv::FileNode& root, const char* name, std::optional<double>& value, std::string& error_message)
{
    const auto node = root[name];
    if (node.empty())
        return true;
    ...
    {
        error_message = std::string("field must be a number: ") + name;
        return false;
    }
    value = (double)node;
    return true;
...
bool readBool(const cv::FileNode& root, const char* name, std::optional<bool>& value, std::string& error_message)
{
    const auto node = root[name];
    if (node.empty())
    {
...
ReadResult JsonLineReader::read()
{
    ...
    if (line.empty())
    {
        return ReadResult{ReadStatus::invalid, {}, std::nullopt, "invalid_json", "empty JSON line"};
    }
    ...
    try
    {
        ...
        if (!storage.isOpened())
        {
            return ReadResult{ReadStatus::invalid, {}, std::nullopt, "invalid_json", "failed to parse JSON"};
        }

        const auto root = storage.root();
        if (root.empty() || !root.isMap())
        {
            return ReadResult{ReadStatus::invalid, {}, std::nullopt, "invalid_json", "JSON command must be an object"};
        }
        ...
        if (!readString(root, "id", message.id, error_message) ||
            ...
            !readInteger(root, "x", message.x, error_message) || !readInteger(root, "y", message.y, error_message) ||
        ...
        {
            return ReadResult{ReadStatus::invalid, {}, message.id, "invalid_json", std::move(error_message)};
        }
