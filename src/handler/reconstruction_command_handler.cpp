    ...
    std::visit(
        [&](auto&& c)
        {
            ...
            if constexpr (std::is_same_v<T, cmd::CmdReconstruct>)
            {
                ...
                cv::Mat tx = (cv::Mat_<double>(3, 3) << 0, -T.at<double>(2), T.at<double>(1), T.at<double>(2), 0,
                              -T.at<double>(0), -T.at<double>(1), T.at<double>(0), 0);
                cv::Mat E = tx * R;
                cv::Mat F = K2.inv().t() * E * K1.inv();

                auto get_files = [](const std::string& d)
                {
...

                {
                    ...
                    double fx = K1.at<double>(0, 0), fy = K1.at<double>(1, 1), cx = K1.at<double>(0, 2),
                           cy = K1.at<double>(1, 2);
                    for (const auto& p : pts3d)
                    {
                        if (p[2] <= 0)
