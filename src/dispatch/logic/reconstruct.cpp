...
void run_reconstruction(runtime::AppContext& ctx, const cmd::CmdReconstruct& c, win::Window& target_window)
{
    ...
    try
    {
        ...
        if (!fs.isOpened())
        {
            LOG_ERROR("File Open Error: {}", c.calib_file);
            return;
        }
        fs["K1"] >> K1;
        fs["D1"] >> D1;
    ...
    cv::Mat tx = (cv::Mat_<double>(3, 3) << 0, -T.at<double>(2), T.at<double>(1), T.at<double>(2), 0, -T.at<double>(0),
    ...
    auto fL = get_files(c.scan_dir_L);
    auto fR = get_files(c.scan_dir_R);
    if (fL.empty())
    {
    ...
    for (size_t i = 0; i < fL.size(); ++i)
    {
        ...
        const uint8_t *pL = imgL.data, *pR = imgR.data, *pMinL = minL.data, *pMaxL = maxL.data, *pMinR = minR.data,
                      *pMaxR = maxR.data;
