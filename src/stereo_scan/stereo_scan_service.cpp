...
OperationResult operationFailure(const std::string& fallback_code, const std::string& fallback_message,
                                 const auto& error)
{
    return OperationResult::failure(error ? error->code : fallback_code, error ? error->message : fallback_message);
}

class DomainStereoScanBackend final : public StereoScanBackend
{
  public:
    DomainStereoScanBackend(video::CameraService& cameras, win::WindowService& windows, win::MonitorService& monitors,
                            projector::ProjectorService& projectors, scan::ScanService& scans,
                            decode::DecodeService& decoder, reconstruction::ReconstructionService& reconstruction,
    ...
    {
        if (const auto existing = stream_lookup_(role))
        ...
        if (!result.ok)
            return {{false, result.error ? result.error->code : "stream_start_failed",
                     result.error ? result.error->message : "stream start failed"},
                    false,
                    {}};
    ...
    MonitorSizeResult monitorSize(int monitor_index) override
    {
        ...
        if (!monitor)
            return {{false, "monitor_not_found", "monitor index is unavailable"}, 0, 0};
    ...
    {
        const auto result =
            windows_.openWindow({command.window_role, "Projector", width, height, command.monitor_index, true});
    ...
    {
        const auto result = projectors_.openProjector(
            {command.projector_role, command.window_role, command.code_width, command.code_height});
        return result.ok ? OperationResult::success()
    ...
    {
        const auto result = projectors_.configureSurface(
            {command.projector_role, command.monitor_index, width, height, command.x, command.y,
             command.placement == "custom" ? projector::ProjectorPlacement::custom
                            ...
                            std::stop_token token) override
    {
        ...
        config.sync_mode =
            command.sync_mode == "photodiode" ? scan::ScanSyncMode::photodiode : scan::ScanSyncMode::delay;
        ...
        while (!token.stop_requested())
        {
            ...
            if (status.status == scan::ScanState::completed)
                                     ...
                                     const std::filesystem::path& decode_dir) override
    {
        const auto result = reconstruction_.reconstruct(
            {{decode_dir, command.calibration_file, {command.max_epipolar_error_px, {}, {}}}, command.ply_file, false});
        if (!result.ok)
            return {{false, result.error ? result.error->code : "triangulation_failed",
                     result.error ? result.error->message : "reconstruction failed"},
                    {},
                    0};
        return {{true, {}, {}}, result.output_file, result.point_count};
    }

    void closeProjector(const std::string& role) override
    {
        (void)projectors_.closeProjector(role);
    }
    void closeWindow(const std::string& role) override
    {
        (void)windows_.closeWindow(role);
    }
    void stopStream(const std::string& role) override
    {
        (void)stop_stream_(role);
    }
    void closeCamera(const std::string& role) override
    {
        (void)cameras_.closeCamera(role);
    }
    void requestStopScan(const std::string& scan_id) override
    {
        (void)scans_.stopScan(scan_id);
    }
...
StereoScanService::StereoScanService(video::CameraService& cameras, win::WindowService& windows,
                                     ...
                                     StreamLookup stream_lookup)
    : owned_backend_(std::make_unique<DomainStereoScanBackend>(cameras, windows, monitors, projectors, scans, decoder,
                                                               reconstruction, std::move(start_stream),
                                                               std::move(stop_stream), std::move(stream_lookup))),
...
common::CommandResult StereoScanService::start(const cmd::CmdStereoScan& command)
{
    ...
    if (active_)
        return common::failure("stereo_scan_already_running", "a stereo scan is already running");
    if (worker_.joinable())
    {
        lock.unlock();
        worker_ = std::jthread{};
    ...
    if (!validateCalibration(command.calibration_file, code, message))
    {
        push(
            "stereo_scan_failed",
            {{"scan_id", command.scan_id}, {"stage", "calibration"}, {"error_code", code}, {"error_message", message}});
    ...
    if (!monitor.ok)
        return common::failure(monitor.error_code, monitor.error_message);
    ...
    try
    {
        ...
        if (command.ply_file.has_parent_path())
            std::filesystem::create_directories(command.ply_file.parent_path());
    }
    catch (const std::exception& error)
    {
        return common::failure("directory_create_failed", error.what());
    }
    ...
    if (!left_camera.ok)
    {
        push("stereo_scan_failed", {{"scan_id", command.scan_id},
                                    {"stage", "camera"},
                                    {"error_code", left_camera.error_code},
                                    {"error_message", left_camera.error_message}});
    ...
    if (!right_camera.ok)
    {
        ...
        push("stereo_scan_failed", {{"scan_id", command.scan_id},
                                    {"stage", "camera"},
                                    {"error_code", right_camera.error_code},
                                    {"error_message", right_camera.error_message}});
    ...
    if (!left_stream.ok)
    {
        ...
        push("stereo_scan_failed", {{"scan_id", command.scan_id},
                                    {"stage", "stream"},
                                    {"error_code", left_stream.error_code},
                                    {"error_message", left_stream.error_message}});
    ...
    if (!right_stream.ok)
    {
        ...
        push("stereo_scan_failed", {{"scan_id", command.scan_id},
                                    {"stage", "stream"},
                                    {"error_code", right_stream.error_code},
                                    {"error_message", right_stream.error_message}});
    ...
    worker_ = std::jthread(
        [this, command, left_url = left_stream.value, right_url = right_stream.value, owned, display_width,
         display_height](std::stop_token token) mutable
        {
            InactiveGuard inactive([this] { setInactive(); });
            try
            {
                workerLoop(token, command, left_url, right_url, owned, display_width, display_height);
            }
            catch (const std::exception& error)
            {
                fail(command, owned, "scan", "internal_error", error.what(), true);
            }
            catch (...)
            {
                fail(command, owned, "scan", "internal_error", "unhandled stereo scan error", true);
            }
        });
    push("stereo_scan_started", {{"scan_id", command.scan_id},
                                 {"left_stream_url", left_stream.value},
                                 {"right_stream_url", right_stream.value},
                                 {"output_dir", command.output_dir.string()}});
    return common::success({{"scan_id", command.scan_id},
                            {"left_stream_url", left_stream.value},
                            {"right_stream_url", right_stream.value},
                            {"output_dir", command.output_dir.string()},
...
void StereoScanService::workerLoop(std::stop_token token, cmd::CmdStereoScan command, std::string left_url,
                                   std::string right_url, StereoScanOwnedResources owned, int display_width,
                                   int display_height)
{
    auto result = backend_.openWindow(command, display_width, display_height);
    if (!result.ok)
    {
        fail(command, owned, "window", result.error_code, result.error_message, true);
        return;
    }
    ...
    if (!result.ok)
    {
        fail(command, owned, "projector", result.error_code, result.error_message, true);
        return;
    }
    ...
    if (!result.ok)
    {
        fail(command, owned, "scan", result.error_code, result.error_message, true);
        return;
    }

    // Window/Projector are scan-only resources. Release them before file-only stages.
    backend_.closeProjector(command.projector_role);
    owned.opened_projector = false;
    backend_.closeWindow(command.window_role);
    owned.opened_window = false;
    ...
    if (!result.ok)
    {
        fail(command, owned, "decode", result.error_code, result.error_message, true);
        return;
    }
    ...
    push("stereo_scan_completed", {{"scan_id", command.scan_id},
                                   {"left_stream_url", left_url},
                                   {"right_stream_url", right_url},
                                   {"scan_dir", (command.output_dir / "scan").string()},
                                   {"decode_dir", (command.output_dir / "decode").string()},
                                   {"ply_file", reconstruction.ply_file.string()},
...
{
    if (owned.opened_projector)
        backend_.closeProjector(command.projector_role);
    if (owned.opened_window)
        backend_.closeWindow(command.window_role);
    if (!cleanup_camera_stream)
        return;
    if (owned.started_right_stream)
        backend_.stopStream(command.right_role);
    if (owned.started_left_stream)
        backend_.stopStream(command.left_role);
    if (owned.opened_right_camera)
        backend_.closeCamera(command.right_role);
    if (owned.opened_left_camera)
        backend_.closeCamera(command.left_role);
}

void StereoScanService::fail(const cmd::CmdStereoScan& command, const StereoScanOwnedResources& owned,
                             const std::string& stage, const std::string& code, const std::string& message,
                             bool cleanup_camera_stream)
{
    cleanup(command, owned, cleanup_camera_stream);
    push("stereo_scan_failed",
         {{"scan_id", command.scan_id}, {"stage", stage}, {"error_code", code}, {"error_message", message}});
}

bool StereoScanService::validateCalibration(const std::filesystem::path& path, std::string& code,
                                            std::string& message) const
{
    if (!std::filesystem::exists(path))
    {
        code = "stereo_calibration_file_not_found";
        message = "stereo calibration file not found: " + path.string();
        return false;
    }
    try
    {
        ...
        if (!storage.isOpened() || k1.rows != 3 || k1.cols != 3 || k2.rows != 3 || k2.cols != 3 || d1.empty() ||
            d2.empty() || r.rows != 3 || r.cols != 3 || t.total() != 3 || width <= 0 || height <= 0)
        {
            code = "stereo_calibration_file_invalid";
            message = "stereo calibration file is missing K/D, R/T, or image size";
            return false;
        }
    }
...
void StereoScanService::requestShutdown()
{
    ...
    if (!scan_id.empty())
        backend_.requestStopScan(scan_id);
}
