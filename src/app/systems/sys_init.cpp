...
void setup_app(runtime::AppContext& ctx)
{
    ...
    if (ctx.id_cam1 != video::kInvalidCameraId)
        ctx.cam_to_win[ctx.id_cam1] = ctx.id_preview;

    video::CameraOptions opt2;
    opt2.device_index = 6;
    ctx.id_cam2 = ctx.cam_mgr.createCamera(opt2, "CamRight");
    if (ctx.id_cam2 != video::kInvalidCameraId)
        ctx.cam_to_win[ctx.id_cam2] = ctx.id_second;
    ...
    auto get_focused_mono = [&ctx, dir_mono_L, dir_mono_R]() -> std::pair<video::CameraId, std::string>
    {
        ...
        {
            if (ctx.cam_to_win[ctx.scan_cam_id_left] == ctx.focused_id)
                return {ctx.scan_cam_id_left, dir_mono_L};
        }
        if (ctx.scan_cam_id_right != video::kInvalidCameraId)
        {
            if (ctx.cam_to_win[ctx.scan_cam_id_right] == ctx.focused_id)
                return {ctx.scan_cam_id_right, dir_mono_R};
        }
        return {video::kInvalidCameraId, ""};
    };

    // バインド登録
    input::install_default_bindings(ctx.input, ctx.cmd_que, ctx.id_projector, calib_cfg, get_focused_mono);
