...
#include "app/dispatch.hpp" // saveScanResultsなどをコマンド化するか、ここで実装するか
...
void update_preview(AppContext& ctx) {
    ctx.cam_mgr.forEach([&](video::Camera& cam){
        ...
        if (ctx.calibrator && ctx.show_chess_corners && !is_scanning) {
            ...
            if (!vis.empty()) display = vis;
        }

        if (ctx.cam_to_win.count(cam.id())) {
            auto wid = ctx.cam_to_win[cam.id()];
            if (auto* w = ctx.win_mgr.get(wid)) w->setImage(display);
        }
    });
}
...
// スキャンロジック (保存ヘルパー含む)
// -----------------------------------------------------------------------------
static void save_scan_results(AppContext& ctx) {
    if (ctx.scanned_imgs_left.empty() && ctx.scanned_imgs_right.empty()) return;
...
void update_scan(AppContext& ctx) {
    if (!ctx.sl_system || !ctx.sl_system->isScanning()) return;
    ...
    if (ctx.sl_system->checkTimerAndReset(ctx.scan_interval_ms)) {
        ...
        auto capture = [&](video::CameraId cid, std::vector<cv::Mat>& buf) {
            ...
            if (cid != video::kInvalidCameraId) {
                if (auto* c = ctx.cam_mgr.get(cid)) f = c->getFrame();
            }
