#include "app/dispatch/logic/reconstruct.hpp"
...
void run_reconstruction(AppContext& ctx, const cmd::CmdReconstruct& c, win::Window& target_window) {
    if (target_window.id() != ctx.id_preview) return;
    ...
    try {
        cv::FileStorage fs(c.calib_file, cv::FileStorage::READ);
        if(!fs.isOpened()) { LOG_ERROR("File Open Error: {}", c.calib_file); return; }
    ...
    cv::Mat R_mat, T_mat; R.copyTo(R_mat); T.copyTo(T_mat);
    cv::Mat RT; cv::hconcat(R_mat, T_mat, RT);
    cv::Mat P2 = K2 * RT; 
    ...
    // 3. 画像読み込み & デコード
    auto get_files = [](std::string d){ 
        std::vector<std::string> f; 
        if(fs::exists(d)) {
            for(auto& e:fs::directory_iterator(d)) if(e.is_regular_file()) f.push_back(e.path().string());
        }
        std::sort(f.begin(),f.end()); 
        return f; 
    };

    auto fL = get_files(c.scan_dir_L); auto fR = get_files(c.scan_dir_R);
    if(fL.empty()) { LOG_ERROR("画像が見つかりません"); return; }
    ...
    for(size_t i=0; i<fL.size(); ++i) {
        if(i>=64) break;
        uint64_t bit = 1ULL << i;
        cv::Mat imgL = cv::imread(fL[i], 0); cv::Mat imgR = cv::imread(fR[i], 0);
        cv::min(minL, imgL, minL); cv::max(maxL, imgL, maxL);
        cv::min(minR, imgR, minR); cv::max(maxR, imgR, maxR);
        
        const uint8_t *pL=imgL.data, *pR=imgR.data, *pMinL=minL.data, *pMaxL=maxL.data, *pMinR=minR.data, *pMaxR=maxR.data;
        for(int p=0; p<sz.area(); ++p) {
            if(pL[p] > (pMinL[p]+pMaxL[p])/2) codeL[p] |= bit;
            if(pR[p] > (pMinR[p]+pMaxR[p])/2) codeR[p] |= bit;
        }
    }

    // 4. マスク生成 & 可視化画像作成 (閾値緩和版)
    int contrast_th = 1; // ★緩和設定
    ...
    for(int i=0; i<sz.area(); ++i) {
        ...
        if(maxL.data[i] - minL.data[i] > contrast_th) maskL.data[i] = 255;
        if(maxR.data[i] - minR.data[i] > contrast_th) maskR.data[i] = 255;
    ...
    for(int y=0; y<sz.height; y++) {
        for(int x=0; x<sz.width; x++) {
            if(!maskL.at<uint8_t>(y,x)) continue;
            ...
            for(int i=0; i<46; ++i) search_codes.push_back(c_origin ^ (1ULL << i));
            ...
            // B. エピポーラ線
            double a = F.at<double>(0,0)*pL.x + F.at<double>(0,1)*pL.y + F.at<double>(0,2);
            double b = F.at<double>(1,0)*pL.x + F.at<double>(1,1)*pL.y + F.at<double>(1,2);
            double c_line = F.at<double>(2,0)*pL.x + F.at<double>(2,1)*pL.y + F.at<double>(2,2);
            double norm = std::sqrt(a*a + b*b);

            // C. 右画像探索
            cv::Point2f best_pR_raw;
            double min_dist = 20.0; // ★緩和設定
            bool found = false;

            for (uint64_t c_target : search_codes) {
                auto it = right_code_map.find(c_target);
                if (it == right_code_map.end()) continue;
            ...
            if(found) {
                matched_count++;
                if (matched_count % 100 == 0) { // 間引き描画
                    cv::circle(debugMatchL, cv::Point(x, y), 1, cv::Scalar(0, 255, 0), -1);
    ...
    LOG_INFO("Reconstruct: マッチング候補数: {}, 3D化成功数: {}", matched_count, pts3d.size());
    ...
    // 6. 保存
    {
        std::ofstream ply(c.output_ply);
        ply << "ply\nformat ascii 1.0\nelement vertex " << pts3d.size() << "\nproperty float x\nproperty float y\nproperty float z\nend_header\n";
        for(const auto& p : pts3d) ply << p[0] << " " << p[1] << " " << p[2] << "\n";
        LOG_INFO("Saved PLY: {}", c.output_ply);
    }

    {
        ...
        double fx = K1.at<double>(0,0), fy = K1.at<double>(1,1), cx = K1.at<double>(0,2), cy = K1.at<double>(1,2);
        for(const auto& p : pts3d) {
            if (p[2] <= 0) continue;
        ...
        cv::Mat mask; cv::compare(depth_map, 0, mask, cv::CMP_EQ);
    ...
    {
        cv::FileStorage fs("reconstruction_points.yml", cv::FileStorage::WRITE);
        if(fs.isOpened()) {
            fs << "points" << cv::Mat(pts3d);
            fs.release();
