...
bool handle_global(AppContext& ctx, const cmd::Command& command) {
    ...
    std::visit([&](auto&& c){
        ...
        if constexpr (std::is_same_v<T, cmd::CmdFocusNext>) {
            ...
            ctx.win_mgr.forEach([&](win::Window& w) { if (w.visible()) ids.push_back(w.id()); });
            if (!ids.empty()) {
...
void handle_window(AppContext& ctx, win::Window& target_window, const cmd::Command& command) {
    std::visit([&](auto&& c){
        ...
        // スキャン制御
            ...
            // (ここは長いですが、必要なら同様に logic/scan.cpp に切り出せます)
            if (!ctx.sl_system) return;
            std::string dir_L = "captures/scan_L", dir_R = "captures/scan_R";
            try {
                fs::remove_all(dir_L); fs::remove_all(dir_R);
                fs::create_directories(dir_L); fs::create_directories(dir_R);
            } catch (...) { return; }
            ...
            while (true) {
                ...
                cv::Mat img_L, img_R;
                if (auto* cam = ctx.cam_mgr.get(ctx.scan_cam_id_left))  img_L = cam->getFrame();
                if (auto* cam = ctx.cam_mgr.get(ctx.scan_cam_id_right)) img_R = cam->getFrame();
                if (auto* w = ctx.win_mgr.get(ctx.id_preview)) { if(!img_L.empty()){ w->setImage(img_L); w->present(); } }
                if (auto* w = ctx.win_mgr.get(ctx.id_second))  { if(!img_R.empty()){ w->setImage(img_R); w->present(); } }
                cv::waitKey(1);
                if (!img_L.empty() && !img_R.empty()) {
                    ...
                    cv::imwrite(ss_L.str(), img_L); cv::imwrite(ss_R.str(), img_R);
                }
                count++;
                int prev = ctx.sl_system->getCurrentIndex();
                ctx.sl_system->nextPattern(false);
                if (ctx.sl_system->getCurrentIndex() <= prev) break; 
            }
            ...
            if (ctx.sl_system) ctx.sl_system->stopScan();
        }

        // キャリブレーション支援
        else if constexpr (std::is_same_v<T, cmd::CmdCalibClear>) {
            if (target_window.id() == ctx.id_preview && !c.target_directory.empty()) {
                fs::remove_all(c.target_directory); fs::create_directories(c.target_directory);
            ...
            if (it != ctx.cam_to_win.end() && it->second != target_window.id()) return;
            ...
            if (auto* cam = ctx.cam_mgr.get(c.camera_id)) {
                ...
                if (!frame.empty()) {
                    std::stringstream ss;
                    ss << c.target_directory << "/" << c.prefix << std::setfill('0') << std::setw(3) << cnt << ".png";
            ...
            if(fs::exists(c.image_folder)) for(auto& p: fs::directory_iterator(c.image_folder)) if(p.is_regular_file()) files.push_back(p.path().string());
            if(files.empty()) return;
            cv::Mat K, D;
            double rms = ctx.calibrator->runCalibration(files, K, D);
            if(rms > 0 && rms < 1.0) {
                if(auto* cam = ctx.cam_mgr.get(c.target_camera_id)) { cam->setIntrinsics(K); cam->setDistCoeffs(D); }
                LOG_INFO("Calib: 成功 RMS={}", rms);
            }
        }
        else if constexpr (std::is_same_v<T, cmd::CmdCapturePush>) { /* 省略 */ } 
        else if constexpr (std::is_same_v<T, cmd::CmdStereoCalibrate>) {
             if (!ctx.stereo_calibrator) return;
             auto* cL = ctx.cam_mgr.get(c.left_cam_id); auto* cR = ctx.cam_mgr.get(c.right_cam_id);
             if(!cL || !cR) return;
             cv::Mat K1=cL->intrinsics(), D1=cL->distCoeffs(), K2=cR->intrinsics(), D2=cR->distCoeffs();
             if(K1.empty() || K2.empty()) { LOG_ERROR("Stereo: 単眼パラメータ不足"); return; }
             
             auto getf = [](std::string d){ std::vector<std::string> f; if(fs::exists(d)) for(auto& e:fs::directory_iterator(d)) if(e.is_regular_file()) f.push_back(e.path().string()); std::sort(f.begin(),f.end()); return f; };
             auto fL = getf(c.left_dir); auto fR = getf(c.right_dir);
             if(fL.size() != fR.size() || fL.empty()) { LOG_ERROR("Stereo: 画像数不一致"); return; }
             ...
             if(rms > 0 && res.valid) {
                 ...
                 if(fs.isOpened()) {
                     fs << "RMS" << rms << "K1" << K1 << "D1" << D1 << "K2" << K2 << "D2" << D2
                        << "R" << res.R << "T" << res.T << "Q" << res.Q;
                 }
             }
        }
        // =====================================================================
        ...
        // =====================================================================
            ...
            // ★ここがスッキリしました
            logic::run_reconstruction(ctx, c, target_window);
        }
